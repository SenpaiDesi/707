...
class Feedback(discord.ui.Modal, title="feedback",):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        util.write_log(f"{interaction.user.id} ({interaction.user.name}#{interaction.user.discriminator} Sent feedback.)")
        return await interaction.response.send_message("Thanks for your feedback! We received it.")

    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        print(error)
        util.write_log(f"{current_date_pretty} Exception occured in feedback: {error}")
        admin = await interaction.client.fetch_user(521028748829786134)
        await admin.send(f"Error occured on feedback. Check log file at {current_date_pretty}")
...
class admins(commands.Cog):
    ...
    async def listguilds(self, interaction: discord.Interaction):
        ...
        util.write_log(f"{interaction.user.id} ({interaction.user.name}#{interaction.user.discriminator} Executed listguilds.)")

    @app_commands.command(name="load", description="Load a module")
    @util.is_bot_admin()
    async def load(self, interaction: discord.Interaction, _module: str):
        if not util.is_bot_developer(interaction.user.id):
            return
        ...
        util.write_log(f"{interaction.user.id} ({interaction.user.name}#{interaction.user.discriminator} Loaded {_module})")
    ...
    async def unload(self, interaction: discord.Interaction, _module: str):
        ...
        util.write_log(f"{interaction.user.id} ({interaction.user.name}#{interaction.user.discriminator} Unloaded {_module})")
    ...
    async def _reload(self, interaction: discord.Interaction, _module: str):
        ...
        util.write_log(message=f"{interaction.user.id} ({interaction.user.name}#{interaction.user.discriminator} Reloaded {_module})")

    @commands.command(name="dbsetup")
    @util.is_bot_admin()
    async def dbsetup(self, ctx):
        util.write_log(message=f"{ctx.author.id} ({ctx.author.name}#{ctx.author.discriminator}) Executed Database setup.")
        database = await util.connect_database()
    ...
    async def modify_bot_admins(self, interaction: discord.Interaction, action: str, user: discord.Member):
        ...
        if action == "add":
            try:
                ...
                await database.commit()
                embed = util.create_simple_embed(title="✅Success", color=discord.Color.random(
            ...
            try:
                ...
                await database.commit()
                embed = util.create_simple_embed(title="✅  Success", color=discord.Color.blue(
                ), field_title=f"Completed {action}", field_content=f"Completed {action} on {user.id}")
                await interaction.response.send_message(embed=embed)
                await database.close()
            except ValueError:
                pass
        util.write_log(f"{interaction.user.id} ({interaction.user.name}{interaction.user.discriminator}) commited {action} on botadmins for {user.name}#{user.discriminator} ({user.id})")
    @commands.command(name="listadmins")
    async def list_admins(self, ctx):
        database = await util.connect_database()
        if not util.is_bot_developer(ctx.author.id):
